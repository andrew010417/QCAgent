...
3. If no key is configured, or the request fails (or the model's JSON doesn't parse into `output_type`), falls back to deterministic logic — but *which* logic is not decided by string-matching `agent.name`. It's driven by two `Agent` fields:
   ...
   `_openai_request` sets `response_format: {"type": "json_object"}` on the OpenAI payload whenever `agent.output_type` is set (`classify`, `report_agent`) — this is a hard API-level guarantee that the completion is valid JSON, not just a prompt instruction the model can ignore under load; before this was added, live responses were observed either skipping JSON entirely (pure Markdown prose, no fence at all) or prepending a prose recap before the fenced JSON block. `Runner.run` also strips Markdown code fences (` ```json ... ``` `) from the model's raw text before `json.loads` (`_strip_code_fence` in `agents.py`) as a second layer of defense — `_strip_code_fence` searches the *whole* response for a fenced block via regex rather than only handling the case where the entire response is the fence, so a stray fence wrapper doesn't trip `json.loads` and silently drop to `fallback_builder` even though the model produced valid structured output.
